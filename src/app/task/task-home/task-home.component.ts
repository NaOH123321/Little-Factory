...
export class TaskHomeComponent implements OnInit {
  ...
  launchUpdateTaskDialog(task: TaskVM) {
    ...
    dialogRef
      ...
      .subscribe(val => {
        if (val) {
          this.store$.dispatch(
            new taskActions.TaskUpdateAction({ ...task, ...val })
          );
        }
      });
  }
  ...
  launchCopyTaskDialog(list: TaskListVM) {
    this.lists$
      ...
      .subscribe(val => {
        if (val) {
          this.store$.dispatch(
            new taskActions.TaskMoveAllAction({
              srcListId: list.id,
              targetListId: val
            })
          );
      ...
      .subscribe(name => {
        if (name) {
          return this.store$.dispatch(
            new taskListActions.TaskListUpdateAction({ ...list, name: name })
          );
        }
      });
  }
  ...
  launchNewListDialog() {
    ...
    dialogRef
      ...
      .pipe(
        take(1),
        filter(n => n),
        withLatestFrom(this.projectId$, (name, projectId) => ({
          name,
          projectId
        })),
        withLatestFrom(
          this.store$.pipe(select(fromRoot.getTaskListTotal)),
          (val, order) => ({ ...val, order })
        )
      )
      .subscribe(val => {
        ...
        return this.store$.dispatch(
          new taskListActions.TaskListAddAction(taskList)
        );
      });
  }
  ...
  launchDelListDialog(list: TaskListVM) {
    ...
    dialogRef
      ...
      .subscribe(val => {
        if (val) {
          return this.store$.dispatch(
            new taskListActions.TaskListDeleteAction(list)
          );
        }
      });
  }
  ...
  handleMove(srcData, list: TaskListVM) {
    switch (srcData.tag) {
      case 'task-item':
        this.store$.dispatch(
          new taskActions.TaskMoveAction({
      ...
      case 'task-list':
        this.store$.dispatch(
          new taskListActions.TaskListSwapAction({
            src: srcData.data,
            target: list
          })
        );
